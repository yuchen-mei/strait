"""
Build the (MU + GLB) -> ReLU -> * scale -> e8m0_quant -> bit8_pack CoreIR graph using
pycoreir (zircon_res_deq_ReLU_quant_fp).

Halide pipeline per input channel c:
    residual_output(c) = mu_hw_input(c) + hw_residual_input(c)
    relu_output(c)     = max(residual_output(c) * dequant_scale, 0)
...
Halide's optimizer uses the identity  max(x*d, 0) * q == (d*q) * max(x, 0)  (valid since
q > 0) to fuse the two muls into a single post-ReLU mul by `scale = dequant_scale * quant_scale`.
This cuts one PE per input lane (matches the previous non-strait flow's PE graph).

Unroll model: the C axis is unrolled by `unroll` at the input (2 input lanes per packed
output lane). Output unroll = unroll / 2.

Per input lane c (4 PEs):
  mu_input_io[c].out       -> add_pe[c].data0
  residual_input_io[c].out -> add_pe[c].data1
  add_pe[c].O0             -> max_pe[c].data0     ; const bf16(0.0)  -> max_pe[c].data1
  max_pe[c].O0             -> mul_pe[c].data0     ; const scale      -> mul_pe[c].data1
  mul_pe[c].O0             -> quant_pe[c].data0   ; const bf16(127)  -> quant_pe[c].data1
...
DEFAULT_SCALE = 0.25  # dequant_scale * quant_scale default (0.5 * 0.5)
...
def _build_graph(unroll: int):
    ...
    top = global_ns.new_module("elementwise_add_mul_relu_mul_quant_bf16", _interface_type(context, unroll))
    ...
    for c in range(unroll):
        ...
        # MU + residual
        defn.connect(mu_io.select("out"), add_pe.select("data0"))
        defn.connect(res_io.select("out"), add_pe.select("data1"))
        # ReLU on the sum
        defn.connect(add_pe.select("O0"), max_pe.select("data0"))
        # Fused scale = dequant_scale * quant_scale, applied after ReLU.
        defn.connect(max_pe.select("O0"), mul_pe.select("data0"))
        # e8m0_quant
...
def _configure(context, instances, unroll: int, tensor_size: int, scale: float):
    ...
    defn = instances["add_pe"][0].module_def
    ...
    mul_inst, mul_w = build_pe_inst.pe_inst_to_bits_with_operands(
        "fp_mul",
        data0=("ext", None),
        data1=("const", build_pe_inst.bf16_bits_from_float(scale)),
    )
...
def build_elementwise_add_mul_relu_mul_quant_bf16_context(
    unroll: int = DEFAULT_UNROLL,
    tensor_size: int = DEFAULT_TENSOR_SIZE,
    scale: float = DEFAULT_SCALE,
):
    """
    Build the zircon_res_deq_ReLU_quant_fp strait design.
    ...
    Args:
        ...
        scale: bf16 mul constant = dequant_scale * quant_scale, applied after ReLU.
    """
    context, top, instances = _build_graph(unroll)
    _configure(context, instances, unroll, tensor_size, scale)
    return context, top


def emit_elementwise_add_mul_relu_mul_quant_bf16_design(
    unroll: int, tensor_size: int, output_path: str, scale: float = DEFAULT_SCALE
):
    context, top = build_elementwise_add_mul_relu_mul_quant_bf16_context(unroll, tensor_size, scale)
    out_file = os.path.join(output_path, "design_top.json")
    top.save_to_file(out_file)
    print(f"[INFO] Wrote elementwise_add_mul_relu_mul_quant_bf16 design_top.json to {out_file}")
    return out_file


if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Generate elementwise_add_mul_relu_mul_quant_bf16 CoreIR design_top.json")
    ...
    emit_elementwise_add_mul_relu_mul_quant_bf16_design(
        args.unroll, args.tensor_size, args.output_path, args.scale
    )
