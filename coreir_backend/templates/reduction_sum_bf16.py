"""
Build a bf16 sum-reduction tree CoreIR graph using pycoreir. Reduces a 2D
array along its inner dimension, producing one sum per row (num_vecs outputs
total), using two accumulator ponds.

Templated design parameters:
- unroll: number of parallel input lanes (= glb_i, must be a power of 2).
- vec_length: length of one inner vector.
- num_vecs: number of vectors to reduce.
- top_module: name of the emitted top module (e.g. "vector_reduction_fp").

Architecture (per row):
  [unroll input IOs] -> balanced fp_add tree of (unroll - 1) PEs
...
`num_partial_reduction = vec_length // unroll` partial sums are produced per
...
DEFAULT_NUM_VECS = 4
DEFAULT_TOP_MODULE = "vector_reduction_fp"
...
def _build_graph(unroll: int, top_module: str):
    ...
    # Sum-reduction tree: stage 1 takes the `unroll` input IO lanes;
...
def _configure(context, instances, unroll: int, vec_length: int, num_vecs: int,
    ...
    # All tree / accum / final_reduce PEs run fp_add with ext/ext operands.
...
def build_reduction_sum_bf16_context(
    ...
    _configure(context, instances, unroll, vec_length, num_vecs, top_module=top_module)
    return context, top


def emit_reduction_sum_bf16_design(
    unroll: int,
    vec_length: int,
    num_vecs: int,
    output_path: str,
    ...
    context, top = build_reduction_sum_bf16_context(
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Generate reduction_sum_bf16 CoreIR design_top.json")
