"""
Build the mul -> relu -> e8m0_quant -> bit8_pack CoreIR graph using pycoreir (zircon_deq_q_relu_fp).

Halide pipeline per input channel c:
    unpacked_result(c) = e8m0_quant(
        max(mu_hw_input(c) * scale, 0.0),
        bf16_bits(127),
    )
    result(c_pack) = bit8_pack(unpacked_result(2*c_pack + 1), unpacked_result(2*c_pack))

Here `scale` = dequant_scale * quant_scale (both env-var configurable).

Unroll model: the C axis is unrolled by `unroll` at the input; output unroll = unroll / 2.

Per packed lane i (c_even=2i, c_odd=2i+1):
  input_io[c].out   -> mul_pe[c].data0 ; const scale -> mul_pe[c].data1
  mul_pe[c].O0      -> max_pe[c].data0 ; const bf16(0.0) -> max_pe[c].data1
  max_pe[c].O0      -> quant_pe[c].data0 ; const bf16(127) -> quant_pe[c].data1
...
# MU input: the "_mu_" substring triggers add_mu_prefix_to_io in strait.
INPUT_BASE = "mu_hw_input_stencil"
...
def _build_graph(unroll: int):
    ...
    top = global_ns.new_module("elementwise_mul_relu_quant_pack_bf16", _interface_type(context, unroll))
    ...
    for c in range(unroll):
        ...
        defn.connect(max_pe.select("O0"), quant_pe.select("data0"))
...
def build_elementwise_mul_relu_quant_pack_bf16_context(
    unroll: int = DEFAULT_UNROLL,
    tensor_size: int = DEFAULT_TENSOR_SIZE,
    scale: float = DEFAULT_SCALE,
):
    """
    Build the zircon_deq_q_relu_fp strait design.
    ...
    Args:
        ...
        scale: bf16 mul constant (= dequant_scale * quant_scale in the Halide generator).
    """
...
def emit_elementwise_mul_relu_quant_pack_bf16_design(
    unroll: int, tensor_size: int, output_path: str, scale: float = DEFAULT_SCALE
):
    context, top = build_elementwise_mul_relu_quant_pack_bf16_context(unroll, tensor_size, scale)
    out_file = os.path.join(output_path, "design_top.json")
    top.save_to_file(out_file)
    print(f"[INFO] Wrote elementwise_mul_relu_quant_pack_bf16 design_top.json to {out_file}")
    return out_file


if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Generate elementwise_mul_relu_quant_pack_bf16 CoreIR design_top.json")
    ...
    emit_elementwise_mul_relu_quant_pack_bf16_design(args.unroll, args.tensor_size, args.output_path, args.scale)
