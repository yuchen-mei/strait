...
def convert_bin_to_txt(input_path, output_path, target_dtype):
    ...
    try:
        ...
        hex_width = math.ceil(math.log2(CGRA_DATA_WIDTH))

        if target_dtype in ['int8', 'uint8']:
            if target_dtype == 'int8':
                data = tensor_fp32.astype(np.int8).view(np.uint8)
            else:
                data = tensor_fp32.astype(np.uint8)
            ...
            # Numpy has native support for float16
        ...
        with open(output_path, 'w') as f:
            for i in range(0, len(data), 8):
                chunk = data[i:i+8]
                hex_strings = [f"{val:0{hex_width}x}" for val in chunk]
...
if __name__ == "__main__":
    ...
    convert_bin_to_txt(args.input, args.output, args.dtype)